...
import { stackTraceFilter, stringify, defineConstants, isBrowser, canonicalType } from './utils.mjs';
import _debug from 'debug';
const debug = _debug('mocha:runner');
...
var stackFilter = stackTraceFilter();
...
import { constants as errorConstants } from './error-constants.mjs';
...
_uncaught(err) {
  ...
  if (this.allowUncaught && !isBrowser()) {
...
function thrown2Error(err) {
  return new Error(
    `the ${canonicalType(err)} ${stringify(
      err
